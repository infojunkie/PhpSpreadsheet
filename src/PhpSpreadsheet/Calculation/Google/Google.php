...
class Google
{
    ...
    public static function query(array $data, $query, $headers = -1)
    {
        ...
        try {
            ...
            $create = ('CREATE TABLE sheet(_row INTEGER PRIMARY KEY, ' . implode(', ', array_map(fn ($c, $t) => "$c $t", array_keys($columns), array_values($columns))) . ');');
            $calculation->getDebugLog()->writeDebugLog('Evaluating SQLite3 Query: %s', $create);
            $db->query($create);

            // Populate the table.
            $insert = ('INSERT INTO sheet VALUES(' . implode("),\n(", array_map(function ($row, $values) use ($columns) {
                return $row . ', ' . implode(', ', array_map(function ($column, $value) use ($columns) {
                    if (is_array($value)) {
                        $value = Functions::flattenSingleValue($value);
                    }
                    if (null === $value) {
